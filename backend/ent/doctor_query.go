...
func (dq *DoctorQuery) QueryDoctorToDiagnosis() *DiagnosisQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, doctor.DoctorToDiagnosisTable, doctor.DoctorToDiagnosisPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(dq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (dq *DoctorQuery) sqlAll(ctx context.Context) ([]*Doctor, error) {
	...
	if query := dq.withDoctorToDiagnosis; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Doctor, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.DoctorToDiagnosis = []*Diagnosis{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*Doctor)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			Edge: &sqlgraph.EdgeSpec{
				Inverse: false,
				Table:   doctor.DoctorToDiagnosisTable,
				Columns: doctor.DoctorToDiagnosisPrimaryKey,
			},
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(doctor.DoctorToDiagnosisPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, dq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "DoctorToDiagnosis": %v`, err)
		}
		query.Where(diagnosis.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "DoctorToDiagnosis" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.DoctorToDiagnosis = append(nodes[i].Edges.DoctorToDiagnosis, n)
			}
		}
	}
